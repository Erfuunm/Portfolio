   ...
   <nav className='flex flex-col items-center xl:justify-center gap-y-4 fixed h-max bottom-0 mt-auto xl:right-[2%] z-50 top-0 w-full xl:w-16 xl:max-w-md xl:h-screen '>
    ...
    xl:justify-center gap-y-10 px-4 md:px-40 xl:px-0 h-[80px] xl:h-max py-8 bg-white/10 backdrop-blur-sm text-3xl xl:text-xl xl:rounded-full'>
      {navData.map((link , index ) => {
        return(
           <Link className='relative flex items-center group hover:text-accent transition-all duration-300' href={link.path}
            key={index}>
              {/*Tooltip*/}
            <div className='absolute pr-14 right-0 hidden xl: group-hover:flex'>
              <div className='bg bg-white relative flex text-primary items-center p-[6px] rounded-[3px]'>
